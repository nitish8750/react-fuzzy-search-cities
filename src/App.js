...
export default function App() {
  ...
  const fetchCities = () => {
    ...
    setCities(result);
    console.log("the result is" + cities);
  };
  ...
  const fuzzyFinder = (userInput, collection) => {
    var suggestions = [],
      results = [],
      pattern = userInput.split("").join(".*?"),
      re = new RegExp(pattern, "i"),
      i,
      match;

    for (i = 0; i < collection.length; i++) {
      match = re.exec(collection[i]);

      if (match !== null) {
        suggestions.push([match[0].length, match.index, collection[i]]);
      }
    }
  ...
  const inputChangehandler = () => {
    fuzzyFinder(searchRef.current.value, cities);
  };
  ...
  return (
    <div className="App">
      ...
      <input type="text" ref={searchRef} onChange={inputChangehandler} />
      <ul>
        {Array.isArray(cities) ? cities.map((city) => <li>{city}</li>) : ""}
      </ul>
    </div>
  );
}
